...
tile(Monitor *m)
{
  //unsigned int i, n, h, mw, my, ty;
	unsigned int i, n, h, r, g = 0, mw, my, ty; //gappx change
	Client *c;
    ...
    //mw = m->nmaster ? m->ww * m->mfact : 0;
		mw = m->nmaster ? (m->ww - (g = gappx)) * m->mfact : 0; // gappx change
	else
		mw = m->ww;
	for (i = my = ty = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			//h = (m->wh - my) / (MIN(n, m->nmaster) - i);
      r = MIN(n, m>nmaster) - i; // gappx change
      h = (m->wh - my - gappx * (r - 1)) / r; // gappx change
			resize(c, m->wx, m->wy + my, mw - (2*c->bw), h - (2*c->bw), 0);
			if (my + HEIGHT(c) < m->wh)
				//my += HEIGHT(c);
        my += HEIGHT(c) + gappx; // gappx change
		} else {
			//h = (m->wh - ty) / (n - i);
